...
coord timeval_scale(coord c1, double scale)
{
  coord r;
  double val, int_val, frac_val;

  val = ((double) c1.t.tv_sec + 0.000001 * (double) c1.t.tv_usec) * scale;

  frac_val = modf(val, &int_val);
  r.t.tv_sec = (int) int_val;
  r.t.tv_usec = 1000000 * frac_val;
...
double
timeval_to_double(coord c1)
{
  return c1.t.tv_sec + c1.t.tv_usec * 0.000001; 
}
