...
typedef struct plotter {
  ...
  int view_x_orig;
  int view_y_orig;
  int view_height_orig;
  int view_width_orig;

  coord ts_prev_ack;
  coord prev_ack;
  int prev_bytes_acked;
  int prev_window;
  char *ack_color;
...
static char **gettokens_from_string(char *buf);

coord timeval_parse(char *s);
coord unsigned_parse(char *s);
void inject_init_seg(struct plotter *pl, char **tokens, int ntokens);
void inject_seg(struct plotter *pl, char **tokens, int ntokens, struct inject_buffer *ib, int);
void inject_s(struct plotter *pl, char **tokens, int ntokens, struct inject_buffer *ib, int);
void inject_init_ack(struct plotter *pl, char **tokens, int ntokens);
void inject_ack(struct plotter *pl, char **tokens, int ntokens, struct inject_buffer *ib, int);
void inject_a(struct plotter *pl, char **tokens, int ntokens, struct inject_buffer *ib, int);
...
int parse_arg_orient(char *arg_orient, int *x, int *y, unsigned int *width, unsigned int *height, 
		      PLOTTER pl)
{
  int screen_width = DisplayWidth(pl->dpy, ScreenNumberOfScreen(pl->screen));
  int screen_height = DisplayHeight(pl->dpy, ScreenNumberOfScreen(pl->screen)) - 64;
  ...
  *height = screen_height;
  ...
  if (strcmp(arg_orient, "lh") == 0) {
    *width = screen_width / 2;
  } else if (strcmp(arg_orient, "rh") == 0) {
    *x = screen_width / 2;
    *width = screen_width / 2;
  } else if (strcmp(arg_orient, "th") == 0) {
    *height = screen_height / 2;
  } if (strcmp(arg_orient, "bh") == 0) {
    *y = screen_height / 2;
    *height = screen_height / 2;
  } else
      return 0;
...
#define XPLOT_PLOTTER_RECT_FILE "xplot.rect"
#define MAX_TEMP_FILE_LEN 512
#define RECT_FILE_MAX_LINE 128

int restore_plotter_rect_abs(struct plotter *pl) {

  char temp_file[MAX_TEMP_FILE_LEN];
  char *temp_path;

  temp_path = getenv("TEMP");
  if (temp_path == NULL) temp_path = "/tmp";
  strncpy(temp_file, temp_path, MAX_TEMP_FILE_LEN);
  temp_file[MAX_TEMP_FILE_LEN - 1] = 0;  
  if (strlen(temp_file) < MAX_TEMP_FILE_LEN);

  FILE *fp = fopen(XPLOT_PLOTTER_RECT_FILE, "r");
  char buf[RECT_FILE_MAX_LINE + 1];

  if (!fp) {
    fprintf(stderr, "*** error reading plotter rect file.\n");
    return 0;
  }

  fgets(buf, RECT_FILE_MAX_LINE, fp);
  pl_x_left = parse_coord(pl->x_type, buf);
  fgets(buf, RECT_FILE_MAX_LINE, fp);
  pl_x_right = parse_coord(pl->x_type, buf);
  fgets(buf, RECT_FILE_MAX_LINE, fp);
  pl_y_top = parse_coord(pl->y_type, buf);
  fgets(buf, RECT_FILE_MAX_LINE, fp);
  pl_y_bottom = parse_coord(pl->y_type, buf);

  return fclose(fp);
}


int restore_plotter_rect_rel(struct plotter *pl) {

  FILE *fp = fopen(XPLOT_PLOTTER_RECT_FILE, "r");
  char buf[RECT_FILE_MAX_LINE + 1];
  coord new_left, new_right, new_top, new_bottom, diff_x, diff_y;
  ...
  fgets(buf, RECT_FILE_MAX_LINE, fp);
  new_left = parse_coord(pl->x_type, buf);
  fgets(buf, RECT_FILE_MAX_LINE, fp);
  new_right = parse_coord(pl->x_type, buf);
  diff_x = impls[(int) pl->x_type]->subtract(new_right, new_left);
  pl_x_right = impls[(int) pl->x_type]->add(pl_x_left, diff_x);

  fgets(buf, RECT_FILE_MAX_LINE, fp);
  new_top = parse_coord(pl->y_type, buf);
  fgets(buf, RECT_FILE_MAX_LINE, fp);
  new_bottom = parse_coord(pl->y_type, buf);
  diff_y = impls[(int) pl->y_type]->subtract(new_top, new_bottom);
  pl_y_top = impls[(int) pl->y_type]->add(pl_y_bottom, diff_y);

  return fclose(fp);
}


int save_plotter_rect(struct plotter *pl) {

  FILE *fp = fopen(XPLOT_PLOTTER_RECT_FILE, "w");

  if (!fp) {
    fprintf(stderr, "*** error writing plotter rect file.\n");
    return 0;
  }

  if (pl->x_type == TIMEVAL) {    
    /* timeval_unparse_coord() does not work properly */
    fprintf(fp, "%ld.%06ld\n", pl_x_left.t.tv_sec, pl_x_left.t.tv_usec);
    fprintf(fp, "%ld.%06ld\n", pl_x_right.t.tv_sec, pl_x_right.t.tv_usec);
  } else {
...
#define WINDOW_FRAME_WIDTH 4

void move_resize_plotter(struct plotter *pl) {

  int screen_x = XDisplayWidth(pl->dpy, 0);
  int screen_y = XDisplayHeight(pl->dpy, 0) - WINDOW_TITLE_HEIGHT;

  if (pl->view_level_x == 0 || pl->view_level_y == 0) {
    XMoveResizeWindow(pl->dpy, pl->win, 
		      pl->view_x_orig, pl->view_y_orig,
		      pl->view_width_orig,
		      pl->view_height_orig);
  } else {

    int new_width_x = screen_x / pl->view_level_x;
    int new_width_y = screen_y / pl->view_level_y;

    XMoveResizeWindow(pl->dpy, pl->win, 
		      new_width_x * pl->view_pos_x + WINDOW_FRAME_WIDTH,
		      new_width_y * pl->view_pos_y + WINDOW_TITLE_HEIGHT,
		      new_width_x - 2 * WINDOW_FRAME_WIDTH,
		      new_width_y - WINDOW_TITLE_HEIGHT - WINDOW_FRAME_WIDTH);
  }
...
void display_plotter(PLOTTER pl)
{
  ...
  
  {
    ...
    if (arg_orient) 
      parse_arg_orient(arg_orient, &x, &y, &width, &height, pl);
    ...
    pl->view_x_orig = x;
    pl->view_y_orig = y;
    pl->view_width_orig = width;
    pl->view_height_orig = height;
...
double get_drag_diffx_double(PLOTTER pl)
{
	dXPoint dxp1, dxp2; 
	coord x1, x2;

	dxp1 = dXPoint_from_lXPoint(pl->dragstart);
	dxp2 = dXPoint_from_lXPoint(pl->dragend);

	x1 = unmap_coord(pl->x_type, pl_x_left, pl_x_right, pl->size.x, 
			 dxp1.x);
	x2 = unmap_coord(pl->x_type, pl_x_left, pl_x_right, pl->size.x, 
			 dxp2.x);

	return to_double(pl->x_type, x2) - to_double(pl->x_type, x1);
}


double get_drag_diffy_double(PLOTTER pl)
{
	dXPoint dxp1, dxp2; 
	coord y1, y2;
	...
	y1 = unmap_coord(pl->y_type, pl_y_top, pl_y_bottom, pl->size.y, 
			 dxp1.y);
	y2 = unmap_coord(pl->y_type, pl_y_top, pl_y_bottom, pl->size.y, 
			 dxp2.y);

	return to_double(pl->y_type, y2) - to_double(pl->y_type, y1);
}
...
void draw_pointer_marks(PLOTTER pl, GC gc)
{
  ...
  case MEASURE_SLOPE:
    ...
    {
      double duration_s = get_drag_diffx_double(pl);
      double num_bytes = get_drag_diffy_double(pl);
      double tp_bps = (duration_s != 0.0) ? num_bytes * 8.0 / duration_s : 0;
      double tp = tp_bps;
      double data = num_bytes;
      ...
      for (tp_prefix_index = 0; fabs(tp) >= 1000.0 && si_prefixes[tp_prefix_index + 1] != NULL; tp_prefix_index++) {
	tp /= 1000.0;
      }

      for (data_prefix_index = 0; fabs(data) >= 1000.0 && si_prefixes[data_prefix_index + 1] != NULL; data_prefix_index++) {
	data /= 1000.0;
      }

      sprintf(measure_slope_buf, "%f s, %.4f %sbytes, %.4f %sbps", duration_s, 
	      data, si_prefixes[data_prefix_index], 
	      tp, si_prefixes[tp_prefix_index]);

      p.x = (int) pl->size.x;
      p.y = (int) pl->size.y - 22;
      p = lXPoint_from_dXPoint(tomain(pl, dXPoint_from_lXPoint(p)));

      XTextExtents(pl->font_struct, 
		   measure_slope_buf, strlen(measure_slope_buf),
		   &direction, &font_ascent, &font_descent, &xcs);

      XDrawString(pl->dpy, pl->win, gc, p.x - xcs.width, p.y,
				measure_slope_buf, strlen(measure_slope_buf));
    }
    break;
...
int main(int argc, char *argv[])
{
  ...
  char *arg_init_x_range = 0;
  char *arg_init_y_range = 0;
  char *arg_init_x_off = 0;
  char *arg_init_y_off = 0;
 ...
 */
  ...
  for (; i < argc && *argv[i] == '-'; i++)
	...
	if (dpy2 == 0) {
      ...
      else if (strcmp("-xrng", argv[i]) == 0) {
	if (++i < argc) 
	  arg_init_x_range = argv[i];
      }
      
      else if (strcmp("-yrng", argv[i]) == 0) {
	if (++i < argc) 
	  arg_init_y_range = argv[i];	
      }

      else if (strcmp("-xoff", argv[i]) == 0) {
	if (++i < argc) 
	  arg_init_x_off = argv[i];
      }
      
      else if (strcmp("-yoff", argv[i]) == 0) {
	if (++i < argc) 
	  arg_init_y_off = argv[i];	
      }

      else if (strcmp("-orient", argv[i]) == 0) {
	if (++i < argc) 
	  arg_orient = argv[i];	
      }
...

  {
	...
	if (len > 4 && strcmp(&argv[k][len-4], ".zip") == 0) {
	  char *command;
	  command = (char *) malloc(50 + len);
	  if (command != 0) {
	    sprintf(command, "unzip -p %s", argv[k]);
	    fp = popen(command, "r");
	    free(command);
	  }
	} else if ((len > 3 && strcmp(&argv[k][len-3],".gz") == 0)
	    || (len > 5 && strcmp(&argv[k][len-5], ".xplz") == 0)) {
	  char *command;
	  command = (char *) malloc(50 + len);
	  if (command != 0) {
	...
	for (k = i; k < argc; k++) {
	  ...
	  int len;
	  len = strlen(argv[k]);
	  if ((len > 3 && strcmp(&argv[k][len-3],".gz") == 0)
	      || (len > 5 && strcmp(&argv[k][len-5], ".xplz") == 0)) {
	    char *command;
	    command = (char *) malloc(50 + len);
	    if (command != 0) {
...
#define ALLPLOTTERS pl = the_plotter_list ; pl != NULL; pl = pl->next
  for (ALLPLOTTERS) {
    ...
    if (arg_init_x_range) {
      ...
      /* adjust the right x-data range so that the window displays
       * a range of "arg_init_x_range" */

      coord x_range = parse_coord(pl->x_type, arg_init_x_range);
      pl_x_right = pl_x_left;
      pl_x_right = impls[(int) pl->x_type]->add(pl_x_right, x_range);
    }

    if (arg_init_y_range) {

      /* adjust the right y-data range so that the window displays
       * a range of "arg_init_y_range" */

      coord y_range = parse_coord(pl->y_type, arg_init_y_range);
      pl_y_top = pl_y_bottom;
      pl_y_top = impls[(int) pl->y_type]->add(pl_y_top, y_range);
    }

    if (arg_init_x_off) {
      coord x_off = parse_coord(pl->x_type, arg_init_x_off);
      pl_x_right = impls[(int) pl->x_type]->add(pl_x_right, x_off);
      pl_x_left = impls[(int) pl->x_type]->add(pl_x_left, x_off);
    }

    if (arg_init_y_off) {
      coord y_off = parse_coord(pl->y_type, arg_init_y_off);
      pl_y_top = impls[(int) pl->y_type]->add(pl_y_top, y_off);
      pl_y_bottom = impls[(int) pl->y_type]->add(pl_y_bottom, y_off);
    }
...
/* #define SAVE_PRINTOUTS */
    ...
    case KeyPress: {
      ...
      if (ks == XK_s) {
	save_plotter_rect(pl);
	printf("Plotter range saved.\n");
      } else if (ks == XK_r) {
	if (event.xkey.state & ShiftMask) {
	  restore_plotter_rect_rel(pl);
	  printf("Plotter range restored (relative).\n");
	} else {
	  restore_plotter_rect_abs(pl);
	  printf("Plotter range restored (absolute).\n");
	}
	pl->size_changed = 1;
	...
	if (event.xkey.state & ShiftMask) {
	  pl->view_level_y++;
	  if (pl->view_level_x == 0) {
	    pl->view_level_x = 1;
	  }
	  move_resize_plotter(pl);
	  ...
	  for (ALLPLOTTERS) {
	    ...
	    diff_y = impls[(int) pl->y_type]->subtract(pl_y_top, pl_y_bottom);
	    diff_y = impls[(int) pl->y_type]->scale(diff_y, 0.25);
	    
	    pl_y_bottom = impls[(int) pl->y_type]->add(pl_y_bottom, diff_y);
	    pl_y_top = impls[(int) pl->y_type]->add(pl_y_top, diff_y);
	...
	if (event.xkey.state & ShiftMask) {
	  ...
	  for (ALLPLOTTERS) {
	    ...
	    pl_y_bottom = impls[(int) pl->y_type]->subtract(pl_y_bottom, diff_y);
	    pl_y_top = impls[(int) pl->y_type]->subtract(pl_y_top, diff_y);
	...
	if (event.xkey.state & ShiftMask) {
	  ...
	  for (ALLPLOTTERS) {
	    diff_x = impls[(int) pl->x_type]->subtract(pl_x_right, pl_x_left);
	    diff_x = impls[(int) pl->x_type]->scale(diff_x, 0.25);
	    
	    pl_x_left = impls[(int) pl->x_type]->add(pl_x_left, diff_x);
	    pl_x_right = impls[(int) pl->x_type]->add(pl_x_right, diff_x);
	...
	if (event.xkey.state & ShiftMask) {
	  ...
	  for (ALLPLOTTERS) {
	    ...
	    pl_x_left = impls[(int) pl->x_type]->subtract(pl_x_left, diff_x);
	    pl_x_right = impls[(int) pl->x_type]->subtract(pl_x_right, diff_x);
...
static char **gettokens(FILE *fp)
{
  ...
  return gettokens_from_string(buf);
}
...
/*
 * load commands from fp into plotter
 */ 
...
int get_input(FILE *fp, Display *dpy, int lineno, struct plotter *pl)
{
  ...
  int inject_commands = FALSE;
  int inject_command_index = 0;
  struct inject_buffer inject_buf;
...
#define parseerror(s) \
  ...
  if (((int) pl->x_type) < 0 || ((int) pl->y_type) < 0) {
    printf("ffffffffff %d %d -%s-\n", (int) pl->x_type, (int) pl->y_type, tokens[1]);
    parseerror("unknown coord type");
  }

  inject_buffer_init(&inject_buf);
  
  for (;;) {

    if (inject_commands) {

      char *next_command = inject_buf.entries[inject_command_index++];
	...
	continue;
      ...
      tokens = gettokens_from_string(next_command);
    } else {
      lineno++;
      tokens = gettokens(fp);
    }
    ...
    if (mystrcmp(tokens[0], "init_seg") == 0) {
      inject_init_seg(pl, tokens, ntokens);
      inject_commands = TRUE;
      continue;
    } else if (mystrcmp(tokens[0], "seg") == 0) {
      inject_seg(pl, tokens, ntokens, &inject_buf, FALSE);
      inject_commands = TRUE;
      continue;
    } else if (mystrcmp(tokens[0], "s") == 0) {
      inject_s(pl, tokens, ntokens, &inject_buf, FALSE);
      inject_commands = TRUE;
      continue;
    } else if (mystrcmp(tokens[0], "pseg") == 0) {
      inject_seg(pl, tokens, ntokens, &inject_buf, TRUE);
      inject_commands = TRUE;
      continue;
    } else if (mystrcmp(tokens[0], "ps") == 0) {
      inject_s(pl, tokens, ntokens, &inject_buf, TRUE);
      inject_commands = TRUE;
      continue;
    } else if (mystrcmp(tokens[0], "init_ack") == 0) {
      inject_init_ack(pl, tokens, ntokens);
      inject_commands = TRUE;
      continue;
    } else if (mystrcmp(tokens[0], "ack") == 0) {
      inject_ack(pl, tokens, ntokens, &inject_buf, FALSE);
      inject_commands = TRUE;
      continue;
    } else if (mystrcmp(tokens[0], "a") == 0) {
      inject_a(pl, tokens, ntokens, &inject_buf, FALSE);
      inject_commands = TRUE;
      continue;
    } else if (mystrcmp(tokens[0], "ack3") == 0) {
      inject_ack(pl, tokens, ntokens, &inject_buf, TRUE);
      inject_commands = TRUE;
      continue;
    } else if (mystrcmp(tokens[0], "a3") == 0) {
      inject_a(pl, tokens, ntokens, &inject_buf, TRUE);
      inject_commands = TRUE;
      continue;
    }
...
void inject_init_seg(struct plotter *pl, char **tokens, int ntokens) {
  ...
  /* init_seg abs_ts_now abs_seq default_color */

  if (ntokens < 4)
    return;

  pl->ts_prev_seg = timeval_parse(tokens[1]);
  pl->prev_seq = unsigned_parse(tokens[2]);
  pl->prev_seg_len = 0;
  pl->seg_color = tokens[3];
}
...
white
darrow ts_now seq_start
if is_push_segment:
  diamond ts_now seq_end pushcolor
  dot ts_now seq_end pushcolor
else:
  uarrow ts_now seq_end
line ts_now seq_start ts_now seq_end

Output:

init_seg abs_ts_now abs_seq default_color
[p]seg ts_diff seq_start_diff seg_len [color]
[p]s ts_diff

*/

void inject_seg(struct plotter *pl, char **tokens, int ntokens, 
		struct inject_buffer *ib, int is_push_segment) {
  ...
  inject_buffer_add(ib, color);
  
}


void inject_s(struct plotter *pl, char **tokens, int ntokens, 
	      struct inject_buffer *ib, int is_push_segment) {
...
green
line ts_prev prev_ack ts_now prev_ack
if prev_ack == ack
  dtick ts_now prev_ack
  text ts_now ack "3"
else:
  line ts_now prev_ack ts_now ack
  if rtt_dongle_color != NULL: 
    diamond ts_now ack rtt_dongle_color
yellow
line ts_prev prev_window_end ts_now prev_window_end
if prev_ack+prev_window == ack+window:
  line ts_now prev_window_end ts_now window_end
else:
  utick ts_now window_end

==>

init_ack abs_ts_now default_color
ack[3] ts_diff diff_ack window [color]
a ts_diff
...
void inject_init_ack(struct plotter *pl, char **tokens, int ntokens) {
  ...
  /* init_ack abs_ts_now ack default_color */

  if (ntokens < 4)
    return;

  pl->ts_prev_ack = timeval_parse(tokens[1]);
  pl->prev_ack = unsigned_parse(tokens[2]);
  pl->prev_bytes_acked = 0;
  pl->prev_window = 0;
  pl->ack_color = tokens[3];

}


void inject_ack(struct plotter *pl, char **tokens, int ntokens, struct inject_buffer *ib, 
		int is_triple_dupack) {

}


void inject_a(struct plotter *pl, char **tokens, int ntokens, struct inject_buffer *ib, 
	      int is_triple_dupack) {
